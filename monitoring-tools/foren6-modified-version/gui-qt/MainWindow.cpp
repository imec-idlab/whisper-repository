...
QMainWindow(parent), ui(new Ui::MainWindow)
{
...

    {
		...
		nodeInfoTree.sixtopMain = new QTreeWidgetItem(ui->rplNodeInfoTree);
        nodeInfoTree.sixtopMain->setText(0, "6TiSCH 6top");
...
MainWindow::setTargetNodeInfo(const di_node_t * node, const di_dodag_t * dodag, const di_rpl_instance_t * rpl_instance)
{
    ...
    //6top
    QList < QTreeWidgetItem * >sixtopChildren = nodeInfoTree.sixtopMain->takeChildren();

	QTreeWidgetItem *main_child, *neighbor_id_child,*neighbor_child,*cell_id_child,*cell_child;
    foreach(main_child, sixtopChildren){
		if(main_child->childCount() > 0){
			foreach(neighbor_id_child,main_child->takeChildren()){
				if(neighbor_id_child->childCount()>0){
					foreach(neighbor_child,neighbor_id_child->takeChildren()){
						if(neighbor_child->childCount()>0){
							foreach(cell_id_child,neighbor_child->takeChildren()){
								if(cell_id_child->childCount()>0){
									foreach(cell_child,cell_id_child->takeChildren()){
										delete(cell_child);
									}
								}
								delete(cell_id_child);
							}
						}
						delete(neighbor_child);
					}
				}
				delete(neighbor_id_child);
			}
		}
		delete main_child;
	}

	QTreeWidgetItem* neighbor_count = new QTreeWidgetItem(nodeInfoTree.sixtopMain);
	neighbor_count->setText(0,"Number of Neighbors");	
	neighbor_count->setText(1,QString::number(HASH_COUNT(node_get_sixtop_neighbors(node))));
	QTreeWidgetItem* neighbor_item = new QTreeWidgetItem(nodeInfoTree.sixtopMain);
	neighbor_item->setText(0,"Neighbors");
	neighbor_item->setExpanded(true);
	
	sixtop_neighbor* neighbor, *tmp;
	HASH_ITER(hh, node_get_sixtop_neighbors(node), neighbor, tmp) {
		QTreeWidgetItem* n = new QTreeWidgetItem(neighbor_item);
		n->setText(0,QString::number(neighbor->key.neighbor_mac,16));
		n->setExpanded(true);
		QTreeWidgetItem* cell_count = new QTreeWidgetItem(n);
		cell_count->setText(0,"Number of cells");
		cell_count->setText(1,QString::number(HASH_COUNT(neighbor->cells)));
		QTreeWidgetItem* cell_item = new QTreeWidgetItem(n);
		cell_item->setText(0,"Cells");
		cell_item->setExpanded(true);
		struct neighbor_cell *cell,*tmp_cell;
		HASH_ITER(hh,neighbor->cells,cell,tmp_cell){
			QTreeWidgetItem* c = new QTreeWidgetItem(cell_item);
			QTreeWidgetItem* c_slot_offset = new QTreeWidgetItem(c);
			QTreeWidgetItem* c_channel_offset = new QTreeWidgetItem(c);
			QTreeWidgetItem* c_cell_option = new QTreeWidgetItem(c);
			c->setText(0,QString::number(cell->key.cell_id,16));
			c->setExpanded(true);
			c_slot_offset->setText(0,"Slot offset");
			c_slot_offset->setText(1,QString::number(htobe32(cell->key.cell_id) & 0xffff,16).prepend("0x"));
			c_channel_offset->setText(0,"Channel offset");
			c_channel_offset->setText(1,QString::number((htobe32(cell->key.cell_id) & 0xffff0000) >> 16,16).prepend("0x"));			
			c_cell_option->setText(0,"Cell option");
			QString options;
			if(cell->cell_options.tx)
				options.append("TX");
			if(cell->cell_options.rx){
				if(options.isEmpty()){
					options.append("RX");
				}
				else{
			...
			if(cell->cell_options.shared){
				options.append("|SHARED");
			}
			c_cell_option->setText(1,options);
		}
	}        
