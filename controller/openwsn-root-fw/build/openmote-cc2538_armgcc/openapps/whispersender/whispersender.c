...
void whispersender_init() {
    ...
    if(idmanager_getMyID(ADDR_64B)->addr_64b[7] != 0x36) return;	//only mote 4 can send
...
/*    whispersender_sender_vars.timerId = opentimers_create();*/
...
void whispersender_task_cb() {
   ...
   //printf("I am TARGET node %d, enqueueing DATAW packet with counter %d \n",idmanager_getMyID(ADDR_64B)->addr_64b[7],whispersender_vars.counter);
   leds_debug_toggle();
