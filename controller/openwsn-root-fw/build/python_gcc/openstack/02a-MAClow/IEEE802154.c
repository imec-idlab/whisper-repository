...
//					}
//				}
//				//printf("IEEE2 My fake MAC source address is %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x \n", NeighborAddress.addr_64b[0],NeighborAddress.addr_64b[1],NeighborAddress.addr_64b[2],NeighborAddress.addr_64b[3],NeighborAddress.addr_64b[4],NeighborAddress.addr_64b[5],NeighborAddress.addr_64b[6],NeighborAddress.addr_64b[7]);
//
//				packetfunctions_writeAddress(msg,&NeighborAddress,OW_LITTLE_ENDIAN);
//
//		   }else{
...
//		   }
		   packetfunctions_writeAddress(msg,idmanager_getMyID(ADDR_64B),OW_LITTLE_ENDIAN);
       	  break;
//       case COMPONENT_ICMPv6RPL:
//    	   //printf("I am node %d, prepending IEEE header RPL \n", idmanager_getMyID(ADDR_64B)->addr_64b[7]);
...
//				//printf("I am node %d, Ensambling FAKE DIO header \n",idmanager_getMyID(ADDR_64B)->addr_64b[7]);
//				//printf("IEEE My fake MAC source address is %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x \n", temp_src_mac64b.addr_64b[0],temp_src_mac64b.addr_64b[1],temp_src_mac64b.addr_64b[2],temp_src_mac64b.addr_64b[3],temp_src_mac64b.addr_64b[4],temp_src_mac64b.addr_64b[5],temp_src_mac64b.addr_64b[6],temp_src_mac64b.addr_64b[7]);
//
//				packetfunctions_writeAddress(msg,&temp_src_mac64b,OW_LITTLE_ENDIAN);
//		   }else{
//			   packetfunctions_writeAddress(msg,idmanager_getMyID(ADDR_64B),OW_LITTLE_ENDIAN);
//		   }
//		   break;
       case COMPONENT_SIXTOP:
    	   //printf("I am node %d, ENQUEUEING KA\n", idmanager_getMyID(ADDR_64B)->addr_64b[7]);
    	   packetfunctions_writeAddress(msg,idmanager_getMyID(ADDR_64B),OW_LITTLE_ENDIAN);
    	   break;
       default:
...
//		   if  ( (frameType == IEEE154_TYPE_ACK) && (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x07) && (whisper_getState()==6) ){
//
//				open_addr_t      NeighborAddress;
//				for (i=0;i<MAXNUMNEIGHBORS;i++) {
//					if (neighbors_getNeighborEui64(&NeighborAddress, ADDR_64B, i)) { // this neighbor entry is in use
...
//   //only packets of 6P will have isSixtopFake true
//   if (msg->creator == COMPONENT_SIXTOP_RES){
//	   if (msg->isSixtopFake==TRUE){
...
//	   if (msg->isDioFake==TRUE){
//		   temp_8b          |= IEEE154_ACK_NO_ACK_REQ          << IEEE154_FCF_ACK_REQ;
//		   printf("I am WISPER NODE %d, Setting NOACK required in Fake DIO \n", idmanager_getMyID(ADDR_64B)->addr_64b[7]);
//
//	   }else{
//
//		   if (frameType==IEEE154_TYPE_ACK || packetfunctions_isBroadcastMulticast(nextHop)) {
...
//   }
      ...
      if (frameType==IEEE154_TYPE_ACK || packetfunctions_isBroadcastMulticast(nextHop)) {
         temp_8b          |= IEEE154_ACK_NO_ACK_REQ          << IEEE154_FCF_ACK_REQ;
         //printf("I am node %d, Setting ack to NO\n", idmanager_getMyID(ADDR_64B)->addr_64b[7]);
      } else {
         temp_8b          |= IEEE154_ACK_YES_ACK_REQ         << IEEE154_FCF_ACK_REQ;
         //printf("I am node %d, Setting ack to YES\n", idmanager_getMyID(ADDR_64B)->addr_64b[7]);
      }
