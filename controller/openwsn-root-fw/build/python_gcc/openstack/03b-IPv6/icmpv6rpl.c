...
//		if (whisper_getState() == 3){
//			if (whisper_getSentDIOs() == 0){
//				whisper_setSentDIOs(1);
//			//	printf("I am WISHPER node %d, FAKE DIO 1 SEND DONE wait 1 more \n", idmanager_getMyID(ADDR_64B)->addr_64b[7]);
//			}else{
//				if (whisper_getSentDIOs() == 1){
...
void icmpv6rpl_updateMyDAGrankAndParentSelection() {
    ...
    if ((idmanager_getIsDAGroot())==TRUE) {
        ...
        if (icmpv6rpl_vars.myDAGrank!=MINHOPRANKINCREASE) { // test for change so as not to report unchanged value when root
            icmpv6rpl_vars.myDAGrank=MINHOPRANKINCREASE;
            //printf("I am ROOT NODE %d, updating my rank. Current rank %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],icmpv6rpl_vars.myDAGrank);
            return;
        }
    }
    ...
    if (icmpv6rpl_vars.haveParent==TRUE){
    	...
    	neighbors_getNeighborEui64(&NeighborAddress, ADDR_64B, icmpv6rpl_vars.ParentIndex);
        ...
        if (
            ...
            //printf("I am NODE %d, my updated rank with my current parent %d is %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],NeighborAddress.addr_64b[7],icmpv6rpl_vars.myDAGrank);
        }
    }
...
//            }
            ...
            tentativeDAGrank = (uint32_t)neighborRank+rankIncrease;
            neighbors_getNeighborEui64(&NeighborAddress, ADDR_64B, i);
            //printf("I am NODE %d, my tentative rank with node %d would be %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],NeighborAddress.addr_64b[7],tentativeDAGrank);
...
//            	if (idmanager_getMyID(ADDR_64B)->addr_64b[7]==0x67){
//            		leds_debug_toggle();
//            	}
...
//				ERR_6LOWPAN_UNSUPPORTED,
//				(errorparameter_t)13,
//				(errorparameter_t)(icmpv6rpl_vars.dio.rank)
//		);
////	   printf("I am NODE %d, DIO received from %d announced rank is  %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],msg->l2_nextORpreviousHop.addr_64b[7],icmpv6rpl_vars.dio.rank );
////
//   }
   ...
   for (i=0;i<MAXNUMNEIGHBORS;i++) {
      if (neighbors_getNeighborEui64(&NeighborAddress, ADDR_64B, i)) { // this neighbor entry is in use
         if (packetfunctions_sameAddress(&(msg->l2_nextORpreviousHop),&NeighborAddress)) { // matching address
            neighborRank=neighbors_getNeighborRank(i);
            //printf("====\n");
            //printf("I am NODE %d, DIO received from %d announced rank is  %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],NeighborAddress.addr_64b[7],icmpv6rpl_vars.dio.rank );
            ...
            if ((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x61) && (NeighborAddress.addr_64b[7] != 0x6b)){
            	break;
            }

            //node 3 only will connect with node 1
            if ((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x2b) && (NeighborAddress.addr_64b[7] != 0x6b)){
            	break;
            }


            //node 4 only will connect with node 2
            if ((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x67) && (NeighborAddress.addr_64b[7] != 0x61)) {
            	break;
            }

            //node 5 only will connect with node 3
            if ((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x07) && (NeighborAddress.addr_64b[7] != 0x2b)){

            	break;
            }

            //node 6 only will connect with node 4
            if ((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x55) && (NeighborAddress.addr_64b[7] != 0x67)) {

            	break;
            }

            //node 7 only will connect with node 5
            if ((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0xac) && (NeighborAddress.addr_64b[7] != 0x07)) {

            	break;
            }
            //node 8 only will connect with node 6 or 7
            if ((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x36) && (NeighborAddress.addr_64b[7] == 0x6b)) {

            	break;
            }
            if ((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x36) && (NeighborAddress.addr_64b[7] == 0x61)) {

            	break;
            }
            if ((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x36) && (NeighborAddress.addr_64b[7] == 0x2b)) {

            	break;
            }
            if ((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x36) && (NeighborAddress.addr_64b[7] == 0x67)) {

            	break;
            }
            if ((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x36) && (NeighborAddress.addr_64b[7] == 0x07)) {
            ...
            if (
              ...
              (icmpv6rpl_vars.incomingDio->rank - neighborRank) > ((3*DEFAULTLINKCOST-2)*MINHOPRANKINCREASE)
               ...
               //printf("I am NODE %d, DIO received from %d its rank entry was %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],NeighborAddress.addr_64b[7],neighbors_getNeighborRank(i) );
               neighbors_setNeighborRank(i,icmpv6rpl_vars.incomingDio->rank);
               //printf("I am NODE %d, Rank entry updated for %d its rank is now %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],NeighborAddress.addr_64b[7], icmpv6rpl_vars.incomingDio->rank);
...
*/
void icmpv6rpl_sendWhisperDIO(uint8_t target, uint8_t currentParent,uint16_t rankTarget) {
...
//	   };
	   ...
	   icmpv6rpl_vars.dio.rank                  = rankTarget;
	   //icmpv6rpl_vars.dio.rank                  = 2999;
...
//   if(idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x07){
//	   return;
//   }
   ...
   *(msg->payload+3) = icmpv6rpl_vars.dio.rank        & 0xFF;
   ...
   //printf("I am node %d, Enqueuing DIO with rank %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],icmpv6rpl_vars.dio.rank);
    msg->isDioFake=FALSE;
    //send
    if (icmpv6_send(msg)==E_SUCCESS) {
        icmpv6rpl_vars.busySendingDIO = TRUE; 
