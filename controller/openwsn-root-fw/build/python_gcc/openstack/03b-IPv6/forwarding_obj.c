...
This was done on 2018-03-08 19:45:09.174931.
*/
...
owerror_t forwarding_send(OpenMote* self, OpenQueueEntry_t* msg) {
 ...
 packetfunctions_isAllHostsMulticast(self, &msg->l3_destinationAdd)) {
    ...
    //printf("I am node %d, forwarding \n", idmanager_getMyID(self, ADDR_64B)->addr_64b[7]);

    if (msg->creator == COMPONENT_ICMPv6RPL){
		if( (msg->isDioFake==TRUE) && ( idmanager_getIsDAGroot(self)==TRUE) ){
//			uint8_t whisper_target128[] = {
...
//			};
			...
			//printf("I am node %d, FORWARDING packet DIO FAKE \n", idmanager_getMyID(self, ADDR_64B)->addr_64b[7]);
			//whisper_target128[15]= whisper_getTargetParentOld(self);

//			if ( whisper_getTargetParentOld(self)==0x55){
//				whisper_target128[14]=0x06;
//				whisper_target128[15]=0x55;
//			}else{
//				whisper_target128[14]=0x0a;
//				whisper_target128[15]=0xac;
//			}

			//printf("Setting source to %d in the FAKEDIO\n", whisper_target128[15]);
			// set source address of node 3
			msg->l3_sourceAdd.type=ADDR_128B;
			memcpy(&(msg->l3_sourceAdd.addr_128b[0]),whisper_target128,16);
		}else{
			// set source address (me)
			msg->l3_sourceAdd.type=ADDR_128B;
...
//		memcpy(&(msg->l3_sourceAdd.addr_128b[8]),myadd64->addr_64b,8);
 ...
 iphc_prependIPv6Header(self, msg,
    ...
    //printf("I am node %d, forwarding done\n", idmanager_getMyID(self, ADDR_64B)->addr_64b[7]);
...
void forwarding_getNextHopWhisper(OpenMote* self, open_addr_t* destination128b, open_addr_t* addressToWrite64b) {
...
//	// next hop is always direct
// forwarding_getNextHopWhisper(self, &(msg->l3_destinationAdd),&(msg->l2_nextORpreviousHop));
//
//   }else{
//	   // retrieve the next hop from the routing table
// forwarding_getNextHop(self, &(msg->l3_destinationAdd),&(msg->l2_nextORpreviousHop));
//   }
	   ...
	   if ((msg->isDioFake==TRUE) && ( idmanager_getIsDAGroot(self)==TRUE) ){
		// next hop is always direct
		   ...
		   //printf("I am whisperroot node %d, Sending forwarding internal \n", idmanager_getMyID(self, ADDR_64B)->addr_64b[7]);



			   uint8_t whisperroot_nexthop_to_target[] = {
			   	0xbb, 0xbb, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, \
...
//			uint8_t whisperroot_nexthop_to_target[] = {
//					   0xbb, 0xbb, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, \
//					   0x00, 0x12, 0x4b, 0x00, 0x06, 0x13, 0x0f, 0x00
//				};
//
//
//			if ( whisper_getTargetParentOld(self)==0x55){
//				whisperroot_nexthop_to_target[15]=0x61;
//			}else{
//				whisperroot_nexthop_to_target[15]=0x2b;
//			}


			//whisperroot_nexthop_to_target[15]=2;
			//printf("Setting source to %d in the FAKEDIO\n", whisperroot_target128[15]);
			// set source address of node 3
			msg->l3_sourceAdd.type=ADDR_128B;
			memcpy(&(msg->l3_sourceAdd.addr_128b[0]),whisperroot_nexthop_to_target,16);
 forwarding_getNextHopWhisper(self, &(msg->l3_destinationAdd),&(msg->l2_nextORpreviousHop));

			//whisperroot_nexthop_to_target[14]=0x0f;
		    //whisperroot_nexthop_to_target[15]=0x6b;

		    whisperroot_nexthop_to_target[15]=0x01;

		    //whisperroot_nexthop_to_target[15]=whisperroot_getTargetParentOld();
			//printf("Setting source to %d in the FAKEDIO\n", whisperroot_target128[15]);
			// set source address of node 3
			msg->l3_sourceAdd.type=ADDR_128B;
			memcpy(&(msg->l3_sourceAdd.addr_128b[0]),whisperroot_nexthop_to_target,16);

			printf("I am WISHPER node %d, sending internal \n", idmanager_getMyID(self, ADDR_64B)->addr_64b[7]);


			//printf("I am whisperroot node %d, mac addresses done, source add l3: %d ... %d \n", idmanager_getMyID(self, ADDR_64B)->addr_64b[7],msg->l3_sourceAdd.addr_128b[0],msg->l3_sourceAdd.addr_128b[15]);

	   }else{


		   //printf("I am NODE %d, this packet has source routing... \n", idmanager_getMyID(self, ADDR_64B)->addr_64b[7]);
		   // retrieve the next hop from the routing table
 forwarding_getNextHop(self, &(msg->l3_destinationAdd),&(msg->l2_nextORpreviousHop));

		   //printf("I am NODE %d, next hop is %d\n", idmanager_getMyID(self, ADDR_64B)->addr_64b[7],msg->l2_nextORpreviousHop.addr_64b[7]);
