...
//    len += 1;
    ...
    if (sfid == 5){
    	...
    	if (neighbor->addr_64b[7] == whisper_getTargetParentNew()){
    		//this is the first 6P message

            //printf("A I am node %d, sending FAKE 6P REQUEST to %d with seq num %d and fake source address %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],neighbor->addr_64b[7],sequenceNumber,whisper_getFakeSource());
            //sequenceNumber = sequenceNumber+1;
    	    *((uint8_t*)(pkt->payload)) = sequenceNumber;
    	    len += 1;

    	}if (neighbor->addr_64b[7] == whisper_getTargetChildren()){
    		//this is the first 6P message
    		if (code != IANA_6TOP_CMD_CLEAR){
    			sequenceNumber = sequenceNumber+1;
    		}else{
    	...
    	}if (neighbor->addr_64b[7] == whisper_getTargetParentOld()){
    ...
    if (outcome == E_SUCCESS){
        ...
        case IANA_6TOP_CMD_ADD:
            if (pkt->isSixtopFake==FALSE){
            	...
            	//printf("I am node %d, enqueued ADD to %d, state SIX_STATE_WAIT_ADDREQUEST_SENDDONE %d\n", idmanager_getMyID(ADDR_64B)->addr_64b[7],neighbor->addr_64b[7],sixtop_vars.six2six_state);
            }else{
            	sixtop_vars.six2six_state = SIX_STATE_IDLE;
...
//	      if (idmanager_getIsDAGroot()==TRUE) {
			msg->l2_nextORpreviousHop.addr_64b[6]=0x0f;
        ...
        if(error == E_SUCCESS) {
            ...
            if (msg->l2_sixtop_returnCode == IANA_6TOP_RC_SUCCESS){
                if (msg->l2_sixtop_command == IANA_6TOP_CMD_ADD){
                	...
                	//printf("I am node %d, adding cells  METADATA is %d\n", idmanager_getMyID(ADDR_64B)->addr_64b[7],msg->l2_sixtop_frameID);
                    sixtop_addCells(
...
void sixtop_six2six_notifyReceive(
    ...
    if (type == SIXTOP_CELL_REQUEST){
        ...
        //printf("I am NODE %d, Received a 6P Request from %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],pkt->l2_nextORpreviousHop->addr_64b[7]);

//    	uint8_t          i;
//		open_addr_t      NeighborAddress;
//		for (i=0;i<MAXNUMNEIGHBORS;i++) {
//			if (neighbors_getNeighborEui64(&NeighborAddress, ADDR_64B, i)) { // this neighbor entry is in use
//				if ( packetfunctions_sameAddress(&NeighborAddress,&(pkt->l2_nextORpreviousHop))){
//						//copying l3 64b address to l2
//						//printf("I am node %d, receiving a 6P REQUEST from nieghbor %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],NeighborAddress.addr_64b[7]);
//
//					   //packetfunctions_ip128bToMac64b(&(NeighborAddress.addr_64b),&temp_src_prefix,&temp_src_mac64b);
...
//		}
        ...
        do{
            ...
            //printf("I am node %d, checking seq num. My value is %d, the incoming seqNum is %d\n", idmanager_getMyID(ADDR_64B)->addr_64b[7],neighbors_getSequenceNumber(&(pkt->l2_nextORpreviousHop)),seqNum);
//            if (code == IANA_6TOP_CMD_CLEAR){
...
//            }
            ...
            if (seqNum != neighbors_getSequenceNumber(&(pkt->l2_nextORpreviousHop)) && code != IANA_6TOP_CMD_CLEAR){
                //printf("I am node %d, Received a 6P packet with seq num %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],seqNum);
                //printf("I am node %d, I was expecting seq num %d. Sending ERROR \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],neighbors_getSequenceNumber(&(pkt->l2_nextORpreviousHop)));
        ...
        *((uint8_t*)(response_pkt->payload)) = IANA_6TOP_SUBIE_ID;
        ...
        if (sixtop_vars.isResponseEnabled){
            // send packet
        	//printf("I am NODE %d, Enqueueing response type \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],returnCode);
...
//		}
        ...
        if (code == IANA_6TOP_RC_SUCCESS || code == IANA_6TOP_RC_EOL){
            ...
            default:
            	...
            	if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x99) && (whisper_getState()==6) ){
                	//printf("I am node %d, Removig dedicated cell with old parent \n", idmanager_getMyID(ADDR_64B)->addr_64b[7]);
                	whisper_setState(0);
//                    if (schedule_removeActiveSlot(5,&(pkt->l2_nextORpreviousHop))){
...
bool sixtop_addCells(
    ...
    for(i = 0;i<CELLLIST_MAX_LEN;i++){
        if (cellList[i].isUsed){
            hasCellsAdded = TRUE;
			//printf("I am node %d, adding CELL at offset  %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],cellList[i].slotoffset);



            //6b - 61
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x6b) && (temp_neighbor.addr_64b[7]==0x61) ){
            ...
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x61) && (temp_neighbor.addr_64b[7]==0x6b) ){
                schedule_addActiveSlot(
                    11,
                    type,
                    isShared,
            ...
            //6b - 2b
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x6b) && (temp_neighbor.addr_64b[7]==0x2b) ){
                schedule_addActiveSlot(
                    12,
                    type,
                    isShared,
            ...
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x2b) && (temp_neighbor.addr_64b[7]==0x6b) ){
                schedule_addActiveSlot(
                    12,
                    type,
                    isShared,
            ...
            //61 - 67
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x61) && (temp_neighbor.addr_64b[7]==0x67) ){
                schedule_addActiveSlot(
                    21,
                    type,
                    isShared,
            ...
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x67) && (temp_neighbor.addr_64b[7]==0x61) ){
                schedule_addActiveSlot(
                    21,
                    type,
                    isShared,
            ...
            //2b - 07
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x2b) && (temp_neighbor.addr_64b[7]==0x07) ){
                schedule_addActiveSlot(
                    22,
                    type,
                    isShared,
            ...
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x07) && (temp_neighbor.addr_64b[7]==0x2b) ){
                schedule_addActiveSlot(
                    22,
                    type,
                    isShared,
            ...
            // 67 - 55
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x67) && (temp_neighbor.addr_64b[7]==0x55) ){
                schedule_addActiveSlot(
                    18,
                    type,
                    isShared,
            ...
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x55) && (temp_neighbor.addr_64b[7]==0x67) ){
                schedule_addActiveSlot(
                    18,
                    type,
                    isShared,
            ...
            // 07 - ac
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x07) && (temp_neighbor.addr_64b[7]==0xac) ){
                schedule_addActiveSlot(
                    19,
                    type,
                    isShared,
            ...
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0xac) && (temp_neighbor.addr_64b[7]==0x07) ){
                schedule_addActiveSlot(
                    19,
                    type,
                    isShared,
            ...
            // 55 - 36
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x55) && (temp_neighbor.addr_64b[7]==0x36) ){
                schedule_addActiveSlot(
                    13,
                    type,
                    isShared,
            ...
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x36) && (temp_neighbor.addr_64b[7]==0x55) ){
                schedule_addActiveSlot(
                    13,
                    type,
                    isShared,
            ...
            // ac - 36
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0xac) && (temp_neighbor.addr_64b[7]==0x36) ){
                schedule_addActiveSlot(
                    14,
                    type,
                    isShared,
            ...
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x36) && (temp_neighbor.addr_64b[7]==0xac) ){
                schedule_addActiveSlot(
                    14,
                    type,
                    isShared,
