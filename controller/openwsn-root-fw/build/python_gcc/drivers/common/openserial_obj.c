...
This was done on 2018-03-08 18:34:57.745983.
*/
/**
...
owerror_t openserial_printData(OpenMote* self, uint8_t* buffer, uint8_t length) {
 ...
 outputHdlcWrite(self, buffer[i]);
    ...
    //printf("I am ROOT node %d, MOTE2PCDATA packet\n", idmanager_getMyID(self, ADDR_64B)->addr_64b[7]);
...
void openserial_stop(OpenMote* self) {
 ...
 idmanager_triggerAboutRoot(self);
                break;
            case SERFRAME_PC2MOTE_SETWHISPER:
            	//printf("I am whisperroot node %d, received COMMAND!!! OPENSERIAL \n", idmanager_getMyID(self, ADDR_64B)->addr_64b[7]);
 openbridge_handleWHISPERData(self, &(self->openserial_vars).inputBuf[0],inputBufFill-1);
            	break;
...
void openbridge_handleWHISPERData(OpenMote* self, uint8_t* buf, uint8_t bufLen){

	 printf("I am WISHPER node %d, received WHISPER COMMAND\n", idmanager_getMyID(self, ADDR_64B)->addr_64b[7]);

 whisper_task_remote(self, buf,bufLen);
