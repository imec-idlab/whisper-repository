...
class ParserData(Parser.Parser):
    ...
    UINJECT_MASK    = 'whispersender'
     
    def __init__(self):
        
        # log
        log.debug("create instance WHISPER recv")
    ...
    def parseInput(self,input):
        ...
        #log.debug("received data length {0}".format(len(input)))
...
#            log.debug("packet without source,dest and asn {0}".format(input))
		...
		#print "Mask ok"
	        #log.debug("wrong MASK OK")
                aux      = input[len(input)-20:len(input)-15]  # last 5 bytes of the packet are the ASN in the UDP latency packet
                diff     = self._asndiference(aux,asnbytes)   # calculate difference 
                timeinus = diff*self.MSPERSLOT                # compute time in ms
                SN       = input[len(input)-15:len(input)-13]   # SN sent by mote
		idmote   = input[len(input)-21:len(input)-20]
		#print "source MAC {0}, dest MAC {1}, timeinus, {2},ms, SN {3}, from node {4}".format(source, dest,timeinus,SN,idmote)
		log.debug("source MAC {0}, dest MAC {1}, timeinus, {2}, (ms) SN {3}, from node {4}".format(source, dest,timeinus,SN,idmote))
                if timeinus<0xFFFF:
                    #print "source {0}, dest {1}, timeinus {2}ms, SN {3}".format(source, dest,timeinus,SN)
		    #log.debug("source {0}, dest {1}, timeinus {2}ms, SN {3}".format(source, dest,timeinus,SN))
                    pass
                else:
