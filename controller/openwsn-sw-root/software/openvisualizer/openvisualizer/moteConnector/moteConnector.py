...
class moteConnector(eventBusClient.eventBusClient):
	...
	print "received command: "+str(data['action']) 
		...
		print "received command: "+str(data['action'][1]) 
		val=0
		print str(int(data['action'][2]))
		if int(data['action'][2]) == int(2):
			val=85
		else:
			val=172

		if data['action'][1] == 'whisperrootsend':
		        dataToSend=[87,val,87]
			print "data to send "+str(dataToSend)
		else:
