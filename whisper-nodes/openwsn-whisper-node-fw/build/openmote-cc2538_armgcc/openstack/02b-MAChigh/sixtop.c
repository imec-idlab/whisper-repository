...
//    len += 1;
    ...
    if (sfid == 5){		//this is fake 6P transaction
        // append 6p Seqnum and schedule Generation
        packetfunctions_reserveHeaderSize(pkt,sizeof(uint8_t));
        sequenceNumber              = 0;
    	sfid=0;
...
void sixtop_six2six_sendDone(OpenQueueEntry_t* msg, owerror_t error){
    ...
    if (msg->l2_sixtop_messageType == SIXTOP_CELL_REQUEST){
        if(error == E_FAIL) {
        	...
        	if (msg->isSixtopFake){		//this will only happen in the Whisper node
    ...
    if (type == SIXTOP_CELL_RESPONSE) {
        ...
        if (code == IANA_6TOP_RC_SUCCESS || code == IANA_6TOP_RC_EOL){
            ...
            default:
            	...
            	if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x07) && (whisper_getState()==6) ){
...
bool sixtop_addCells(
    ...
    for(i = 0;i<CELLLIST_MAX_LEN;i++){
	    ...
	    //we assume we know which cells want to add/delete
            ...
            //2b - 67
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x2b) && (temp_neighbor.addr_64b[7]==0x67) ){
                schedule_addActiveSlot(
                    16,
                    type,
                    isShared,
            ...
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x67) && (temp_neighbor.addr_64b[7]==0x2b) ){
                schedule_addActiveSlot(
                    16,
                    type,
                    isShared,
            ...
            //2b - 67
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x07) && (temp_neighbor.addr_64b[7]==0x6b) ){
                schedule_addActiveSlot(
                    10,
                    type,
                    isShared,
            ...
            if( (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x6b) && (temp_neighbor.addr_64b[7]==0x07) ){
                schedule_addActiveSlot(
                    10,
                    type,
                    isShared,
