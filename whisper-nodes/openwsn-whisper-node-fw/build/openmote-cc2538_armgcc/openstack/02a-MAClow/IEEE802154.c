...
void ieee802154_prependHeader(OpenQueueEntry_t* msg,
   ...
   switch(msg->creator){
       case COMPONENT_SIXTOP_RES:
		   ...
		   if (msg->isSixtopFake==TRUE){	//will be run only in Whisper node
       ...
       default:
		   ...
		   if  ( (frameType == IEEE154_TYPE_ACK) && (idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x07) && (whisper_getState()==6) ){	//will be run only in Whisper node
