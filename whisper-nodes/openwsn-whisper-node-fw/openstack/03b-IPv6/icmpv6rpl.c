...
void icmpv6rpl_sendDone(OpenQueueEntry_t* msg, owerror_t error) {
   ...
   if (msg->isDioFake==TRUE){ //will be run only in Whisper node
...
void icmpv6rpl_indicateRxDIO(OpenQueueEntry_t* msg) {
   ...
   for (i=0;i<MAXNUMNEIGHBORS;i++) {
      if (neighbors_getNeighborEui64(&NeighborAddress, ADDR_64B, i)) { // this neighbor entry is in use
	    ...
	    //we filter to for a specific topology (only for testing)

            //node 4 only will connect with node 2 or node 3
            if (((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x67) && (NeighborAddress.addr_64b[7] == 0x6b)) || ((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x67) && (NeighborAddress.addr_64b[7] == 0x07))){
            ...
            if ((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x07) && (NeighborAddress.addr_64b[7] == 0x67)) {
            ...
            if ((idmanager_getMyID(ADDR_64B)->addr_64b[7] == 0x07) && (NeighborAddress.addr_64b[7] == 0x61)) {
...
void icmpv6rpl_sendWhisperDIO(uint8_t target, uint8_t currentParent,uint16_t rankTarget) {
	   ...
	   uint8_t whisper_rpl_destAddress[] = {
		...
		0x00, 0x12, 0x4b, 0x00, 0x06, 0x13, 0x0f, 0x07
	   };
