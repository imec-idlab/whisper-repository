...
void ieee154e_init() {
    ...
    ieee154e_vars.singleChannel     = 12;
...
//		   if (ieee154e_vars.slotOffset==99){
//#			   //printf("ASN %x %x %x --------------------------------------------------------- \n",ieee154e_vars.asn.byte4, ieee154e_vars.asn.bytes2and3 ,ieee154e_vars.asn.bytes0and1);
//			   ////printf("------------------------------------------------------------------------------------------------------------------ \n");
//
//		   }
//	   }
...
port_INLINE void activity_ti1ORri1() {
    ...
    switch (cellType) {
        ...
        case CELLTYPE_TX:
        	...
        	//printf("I am node %d, preparing ti1 at offset %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],ieee154e_vars.slotOffset);
            // stop using serial
            openserial_stop();
                ...
                if (packetfunctions_isBroadcastMulticast(&neighbor)==FALSE){
                    ...
                    ieee154e_vars.dataToSend = openqueue_macGetDedicatedPacket(&neighbor);
                  	 ...
                  	 if (ieee154e_vars.dataToSend!=NULL){
                  		        //printf("I am node %d, transmitting something in DEDICATED at cell %d this the attempt %d\n", idmanager_getMyID(ADDR_64B)->addr_64b[7],ieee154e_vars.slotOffset,ieee154e_vars.dataToSend->l2_numTxAttempts);
...
//									   	    if ( packetfunctions_sameAddress(&NeighborAddress,&(ieee154e_vars.dataToSend->l2_nextORpreviousHop))){
//   									   	    	    //copying l3 64b address to l2
//   												    //printf("I am node %d, sending a 6P Packet in DEDICATED cell offset %d to nieghbor %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],ieee154e_vars.slotOffset,NeighborAddress.addr_64b[7]);
//
...
//   									}
//
//   									        //printf("I am node %d, sending a 6P Packet in MIMIMAL cell offset %d to nieghbor %d \n", idmanager_getMyID(ADDR_64B)->addr_64b[7],ieee154e_vars.slotOffset,neighbor->addr_64b[7]);
//   								}
//   								if (ieee154e_vars.dataToSend->creator==COMPONENT_SIXTOP){
//									    printf("I am node %d, sending a KA Packet in DEDICATED cell offset %d\n", idmanager_getMyID(ADDR_64B)->addr_64b[7],ieee154e_vars.slotOffset);
//
//   								}
   //                							if (ieee154e_vars.dataToSend->creator==COMPONENT_ICMPv6RPL){
   ...
   //                							}
                    ...
                    ieee154e_vars.dataToSend = openqueue_macGetDataPacket(&neighbor);
               	    ...
               	    if (ieee154e_vars.dataToSend!=NULL){
               	    		    //printf("I am node %d, transmitting something in MINIMAL this is the attempt %d\n", idmanager_getMyID(ADDR_64B)->addr_64b[7],ieee154e_vars.dataToSend->l2_numTxAttempts);
...
//
//								if (ieee154e_vars.dataToSend->creator==COMPONENT_ICMPv6RPL){
//									if (ieee154e_vars.dataToSend->isDioFake==TRUE){
//										uint8_t          i;
//										open_addr_t      NeighborAddress;
//										for (i=0;i<MAXNUMNEIGHBORS;i++) {
//											if (neighbors_getNeighborEui64(&NeighborAddress, ADDR_64B, i)) { // this neighbor entry is in use
...
//    if (ieee154e_vars.dataToSend->creator == COMPONENT_SIXTOP_RES){
// 	   if (ieee154e_vars.dataToSend->isSixtopFake==TRUE){
// 		  listenForAck = FALSE;
...
port_INLINE void activity_ti9(PORT_TIMER_WIDTH capturedTime) {
    ...
    */
    do { // this "loop" is only executed once
        ...
        if ( (ieee154e_vars.dataToSend->isSixtopFake) || (ieee154e_vars.dataToSend->isDioFake) ){
...
//            	printf("I am NODE %d, is valid %d \n",idmanager_getMyID(ADDR_64B)->addr_64b[7], ieee802514_header.valid);
//            	printf("I am NODE %d,frame type %d \n",idmanager_getMyID(ADDR_64B)->addr_64b[7], ieee802514_header.frameType);
//            	printf("I am NODE %d, pan ID %d \n",idmanager_getMyID(ADDR_64B)->addr_64b[7], packetfunctions_sameAddress(&ieee802514_header.panid,idmanager_getMyID(ADDR_PANID)));
//            	printf("I am NODE %d, my address %d \n",idmanager_getMyID(ADDR_64B)->addr_64b[7], idmanager_isMyAddress(&ieee802514_header.dest));
//            	//printf("I am NODE %d, same address %d \n",idmanager_getMyID(ADDR_64B)->addr_64b[7], packetfunctions_sameAddress(&ieee802514_header.src,ieee154e_vars.dataToSend->l2_nextORpreviousHop));
//
//            	// break from the do-while loop and execute the clean-up code below
                break;
            }
...
port_INLINE void activity_ri5(PORT_TIMER_WIDTH capturedTime) {
    ...
    */
    do { // this "loop" is only executed once
        ...
        if (idmanager_getMyID(ADDR_64B)->addr_64b[7]==0x07){	//will be run only in Whisper node
        	if (whisper_getState()==6){
...
//   if (ieee154e_vars.dataReceived->creator==COMPONENT_ICMPv6RPL){
